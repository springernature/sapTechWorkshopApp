...
com.springer.workshopapp.util.Controller.extend("com.springer.workshopapp.controller.appController.AuthorDetails", {
	...
	onInit: function() {
		...
		if (sap.ui.Device.system.phone) {
			...
			this.getEventBus().subscribe("AuthorMaster", "InitialLoadFinished", this.onMasterLoaded, this);
		}
	...
	onRouteMatched: function(oEvent) {
		...
		if (oParameters.name !== "Author_detail") {
			return;
		}
...
// check which tab should be selected ->  select the detail tab 
		...
		if (typeof this.sTabKey === "undefined") {
			this.sTabKey = "AuthorMailsSet";
		}
		this.getEventBus().publish("AuthorDetails", "TabChanged", {
	...
	bindView: function(sEntityPath) {
		...
		if (typeof this.sTabKey === "undefined") {
			this.sTabKey = "AuthorDetails";
		}

		switch (this.sTabKey) {
			case "AuthorDetails":
				var sAggregationPath = "/ %ServiceName%"; // odata service
				var oListMails = this.getView().byId("listAuthor");
	...
	fireDetailChanged: function(sEntityPath) {
		this.getEventBus().publish("AuthorDetails", "Changed", {
	...
	fireDetailNotFound: function() {
		this.getEventBus().publish("AuthorDetails", "NotFound");
	},
	...
	onNavBack: function() {
		...
		this.getRouter().myNavBack("Author_master");
	},
	...
	onDetailSelect: function(oEvent) {
		...
		sap.ui.core.UIComponent.getRouterFor(this).navTo("Author_detail", {
